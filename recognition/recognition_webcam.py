...
def grava(id):
    ...
    print(id)
    print(f"Agora: {data_hora_br}")
    conteudo = data_hora_br + "-" + id
...
def recognize_faces(
    network, face_classifier, orig_frame, face_names, threshold, conf_min=0.9
):
    text = ""
    ...
    for i in range(0, detections.shape[2]):
        ...
        if confidence > conf_min:
            ...
            pred_name = (
                face_names[prediction] if conf <= threshold else "Not identified"
            )
...
while True:
    ...
    processed_frame, pred_name = recognize_faces(
        network, face_classifier, frame, face_names, threshold
    )
